...
class ProRe(torch.nn.Module):
    ...
    def forward(self, vote_points, vote_feats):
        '''
        Args:
            vote_points (torch.Tensor): Coordinate of the seed
                points in shape (B, N, 3).
            vote_feats (torch.Tensor): Features of the seed points in shape
                (B, C, N).

        Returns:
            tuple[torch.Tensor]:

                - reasoned_points: reasoned xyz based on the voted points \
                    with shape (B, N, 3).
                - reasoned_features: reasoned features based on the seed points with \
                    shape (B, C', N)
        '''

        num_point = vote_points.shape[1]
        ...
        features = self.conv_channel(features)
        features = P * features
...
class SSD3DHead_ProRe(VoteHead):
